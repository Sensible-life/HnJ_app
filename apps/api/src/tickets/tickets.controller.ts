...
export class TicketsController {
  ...
  approve(@Param('token') token: string, @Res() res: Response) {
    const ticket = this.ticketsStore.resolve(token, 'APPROVED');
    if (!ticket) throw new NotFoundException('유효하지 않은 승인 링크입니다.');
    res.type('html').send(renderApprovalPage(ticket));
  }

  @Post('approve/:token/reinspect')
  reinspect(@Param('token') token: string, @Res() res: Response) {
    const ticket = this.ticketsStore.resolve(token, 'REINSPECT_REQUESTED');
    if (!ticket) throw new NotFoundException('유효하지 않은 승인 링크입니다.');
    res.type('html').send(renderApprovalPage(ticket));
  }
}
